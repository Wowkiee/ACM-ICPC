...
int main() {
	int T;scanf("%d",&T);while(T--) {
		...
		while(q--) {
			...
			int p=lower_bound(all(V),aa)-V.begin();
			if(V[p]==aa) {
				if(a[aa]<b) {
					int pnxt=upper_bound(all(V),b)-V.begin();
					de(Ans);de(pnxt);de(p);
					printf("%d\n",Ans-(pnxt-p)+1);
				}
				else {
					if(b<=V[p-1]) {
						int pos=seg.qry(V[p]+1,V[p+1],V[p-1],1,n,1);
						printf("%d\n",p+ans[pos]-2);
					}
					else {
						int pos=seg.qry(V[p]+1,V[p+1],b,1,n,1);
						printf("%d\n",p+ans[pos]-1);
					}
				}
			}
