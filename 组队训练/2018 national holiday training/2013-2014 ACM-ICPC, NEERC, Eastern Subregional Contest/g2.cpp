...
int main() {
	...
	//--- 1
	rep(i,0,n)rep(j,0,8)data1[i*8+j]=vir(s1[i][j]=='1',0);
	int p=0;
	per(i,0,m)per(j,0,8)data2[p++]=vir(s2[i][j]=='0',0);
	doit(data1,data2);
	rep(i,m,n+1) {
		int ans=data1[i*8-1].re+0.5;
		Ans[i]+=ans;
	}
	//--- 2
	rep(i,0,N)data1[i]=data2[i]=vir(0,0);
	rep(i,0,n)rep(j,0,8)data1[i*8+j]=vir(s1[i][j]=='0',0);
	p=0;
	per(i,0,m)per(j,0,8)data2[p++]=vir(s2[i][j]=='1',0);
	doit(data1,data2);
	rep(i,m,n+1) {
		int ans=data1[i*8-1].re+0.5;
		Ans[i]+=ans;
	...
	int xx=m;
	rep(i,m+1,n+1)if(Ans[i]<Ans[xx])xx=i;
	puts("Yes");
	printf("%d %d\n",Ans[xx],xx-m+1);
	return 0;
}
