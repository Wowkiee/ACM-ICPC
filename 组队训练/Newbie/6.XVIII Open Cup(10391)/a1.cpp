...
struct Simplex {
	...
	db run() {
		...
		rep(i, 1, n + 1) rep(j, 1, m + 1)
			if(B[j] == 1) { ans[i] = (j <= m ? b[j] : 0); break;}
		return v;
	}
}Sim;
int n, a[N][N];	
int main() {
	...
	rep(i, 0, n)rep(j, 0, n) {
		cin >> a[i][j];
		Sim.A[i * n + j][i] = Sim.A[i * n + j][j + n] = 1;
		Sim.b[i * n + j] = 35000 - a[i][j];
	}
	rep(i, 0, n << 1)Sim.c[i] = 1;
	cout << n * (140000 - Sim.run()) << endl;
	rep(i, 0, n << 1)cout << Sim.ans[i] << endl;
	rep(i, 0, n)rep(j, 0, n) {
		cout << 70000 - Sim.ans[i] - Sim.ans[j + n] << " \n"[j == n - 1];
	}
	return 0;
}
