	...
	rep(t, 0, sz(g[u])) {
		...
		if(sz(dcc[v]) < sz(dcc[u])) {
			...
			rep(i, 0, sz(dcc[u])) {
				...
				if(c != -1) {
					if(vis[v]) {
						if(col[v][c] == col[u][i]) ok[dcc[v][c]] = 0;
					} else {
						col[v][c] = (col[u][i] ^ 1);
					}
				}
			}
			if(!vis[v]) rep(i, 0, sz(dcc[v])) if(col[v][i] == -1) col[v][i] = dcc[v][i] << 1;
		}
		if(!vis[v]) dfs(v);
	}
}
bool check(int u, int v) {
	if(sz(dcc[u]) > sz(dcc[v])) swap(u, v);
	rep(i, 0, sz(dcc[u])) {
		int c = find(v, dcc[u][i]);
...
int main() {
	...
	solve();
	fill_n(ok, _, 1);
	rep(i, 0, n) col[i] = vi(sz(dcc[i]), -1);
	rep(i, 0, sz(dcc[0])) col[0][i] = dcc[0][i] << 1;
	dfs(0);
	rep(u, 0, n) {
		rep(i, 0, sz(g[u])) {
			int v = g[u][i];
			if(u < v && check(u, v)) ng[u].pb(v), ng[v].pb(u);
		}
	}
	fill_n(vis, n, 0);
